...
const defaultManagedSubcategories = ['Peripherals', 'Computer Accessories', 'Cables', 'Lighting', 'Kitchen Appliances', 'Productivity Tools', 'Decor'];
...
if (typeof globalThis._managedSubcategoriesStore === 'undefined') { // New
...
if (typeof globalThis._managedRoomsStore === 'undefined') { // New
  globalThis._managedRoomsStore = [...defaultManagedRooms];
}
if (typeof globalThis._managedVendorsStore === 'undefined') { // New
  globalThis._managedVendorsStore = [...defaultManagedVendors];
}
if (typeof globalThis._managedProjectsStore === 'undefined') { // New
...
export async function updateItemStatus(id: string, newStatus: ItemStatus): Promise<Item | undefined> {
  ...
  if (newStatus === 'sold') {
    currentItem.soldDate = currentItem.soldDate || now; // Keep existing soldDate if already sold, else set now
    currentItem.inUseDate = undefined; // Clear inUseDate if sold
  } else if (newStatus === 'in use') {
    currentItem.inUseDate = currentItem.inUseDate || now; // Keep existing inUseDate if already in use, else set now
    currentItem.soldDate = undefined; // Clear soldDate if moved to in use
  } else { // 'in stock'
  ...
  globalThis._itemsStore = store.map(item => {
    if (itemIds.includes(item.id)) {
      if (item.status !== newStatus) {
        ...
        item.updatedAt = now;
        if (newStatus === 'sold') {
          item.soldDate = item.soldDate || now;
          item.inUseDate = undefined;
        } else if (newStatus === 'in use') {
          item.inUseDate = item.inUseDate || now;
...
async function getManagedOptions(storeKey: '_managedCategoriesStore' | '_managedSubcategoriesStore' | '_managedStorageLocationsStore' | '_managedBinLocationsStore' | '_managedRoomsStore' | '_managedVendorsStore' | '_managedProjectsStore'): Promise<string[]> {
  await new Promise(resolve => setTimeout(resolve, 10));
  const store = globalThis[storeKey] || [];
  return [...store].sort();
}

export const getManagedCategoryOptions = () => getManagedOptions('_managedCategoriesStore');
export const getManagedSubcategoryOptions = () => getManagedOptions('_managedSubcategoriesStore');
export const getManagedStorageLocationOptions = () => getManagedOptions('_managedStorageLocationsStore');
export const getManagedBinLocationOptions = () => getManagedOptions('_managedBinLocationsStore');
export const getManagedRoomOptions = () => getManagedOptions('_managedRoomsStore');
export const getManagedVendorOptions = () => getManagedOptions('_managedVendorsStore');
export const getManagedProjectOptions = () => getManagedOptions('_managedProjectsStore');

// --- Managed Options Adders/Deleters ---
async function addManagedOption(
  name: string,
  optionType: string,
  storeKey: '_managedCategoriesStore' | '_managedSubcategoriesStore' | '_managedStorageLocationsStore' | '_managedBinLocationsStore' | '_managedRoomsStore' | '_managedVendorsStore' | '_managedProjectsStore'
...
async function deleteManagedOption(
  ...
  const store = globalThis[storeKey] || [];
  const initialLength = store.length;
  globalThis[storeKey] = store.filter(opt => opt !== name);
...
export const addManagedCategoryOption = (name: string) => addManagedOption(name, "Category", '_managedCategoriesStore');
export const deleteManagedCategoryOption = (name: string) => deleteManagedOption(name, "Category", '_managedCategoriesStore');
export const addManagedSubcategoryOption = (name: string) => addManagedOption(name, "Subcategory", '_managedSubcategoriesStore');
export const deleteManagedSubcategoryOption = (name: string) => deleteManagedOption(name, "Subcategory", '_managedSubcategoriesStore');
export const addManagedStorageLocationOption = (name: string) => addManagedOption(name, "Storage Location", '_managedStorageLocationsStore');
export const deleteManagedStorageLocationOption = (name: string) => deleteManagedOption(name, "Storage Location", '_managedStorageLocationsStore');
export const addManagedBinLocationOption = (name: string) => addManagedOption(name, "Bin Location", '_managedBinLocationsStore');
export const deleteManagedBinLocationOption = (name: string) => deleteManagedOption(name, "Bin Location", '_managedBinLocationsStore');
export const addManagedRoomOption = (name: string) => addManagedOption(name, "Room", '_managedRoomsStore');
export const deleteManagedRoomOption = (name: string) => deleteManagedOption(name, "Room", '_managedRoomsStore');
export const addManagedVendorOption = (name: string) => addManagedOption(name, "Vendor", '_managedVendorsStore');
export const deleteManagedVendorOption = (name: string) => deleteManagedOption(name, "Vendor", '_managedVendorsStore');
export const addManagedProjectOption = (name: string) => addManagedOption(name, "Project", '_managedProjectsStore');
export const deleteManagedProjectOption = (name: string) => deleteManagedOption(name, "Project", '_managedProjectsStore');
