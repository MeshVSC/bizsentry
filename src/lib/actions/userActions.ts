...
export async function loginUser(
  ...
  if (user.password_text === passwordInput) {
    ...
    // console.log("[LoginSuccess] Path revalidated.");
...
export async function getCurrentUser(): Promise<CurrentUser | null> {
  ...
  if (error || !user) {
    if (error && error.code !== 'PGRST116') { // PGRST116 means "Searched for a single row, but 0 rows were found" - which is a valid case if cookie is stale.
